...
| Selected XDG | e.g. prompt, updater, window manager, Goose provider catalogs, Stream Deck action scripts under `~/.config/` | Only non-secret files in the public source |
...
| Stream Deck actions | `~/.config/streamdeck/bin/` from `dot_config/streamdeck/bin/` | `.command` wrappers; live Elgato profiles stay in Application Support (not git) — [stream-deck.md](./stream-deck.md) |
| Himalaya | unmanaged `~/.config/himalaya/config.toml` | Generated by `scripts/himalaya-setup.sh` from gitignored account map; Pass titles only |
...
- Filled `pass-*-map.local` / secret env maps / `himalaya-accounts.local`  
- Generated Mail.app `.mobileconfig` files  
